...
//    self.blackOverlay.frame = CGRectMake(self.contentViewFrame.origin.x, self.contentViewFrame.origin.y+self.contentViewFrame.size.height, self.contentViewFrame.size.width, containerView.height);
    [self show];
}
...
-(void)dismiss
{   if(self.clickedBackCancelHandler)
    {
        self.clickedBackCancelHandler();
    }
    [self dismissWithAnimation:YES];
}
- (void)clickedBackViewToCancel:(dispatch_block_t)clickedbackHandle;
{
    self.clickedBackCancelHandler = clickedbackHandle;
}
