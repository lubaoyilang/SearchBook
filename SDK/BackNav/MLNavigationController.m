...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    _backController = self.topViewController;
    if (_backController != nil) {
        [_titleArray addObject:_backController.navigationItem.title?_backController.navigationItem.title:@""];
    }
    
    _backController.navigationItem.title = @"   ";
...
- (UIViewController *)popViewControllerAnimated:(BOOL)animated
{
    ...
    id obj = [super popViewControllerAnimated:animated];
    
    NSString *titleStr = [_titleArray lastObject];
    
    if (titleStr) {
        
        if ([self.viewControllers count]) {
            UIViewController *base = [self.viewControllers lastObject];
             base.navigationItem.title = titleStr;
    ...
    if (recoginzer.state == UIGestureRecognizerStateBegan) {
        ...
        if (touchPoint.x - startTouch.x > 50)
        {
            [UIView animateWithDuration:0.3 animations:^{
                ...
                BaseViewController *base = (BaseViewController*)[self topViewController];
                if ([base isKindOfClass:[BaseViewController class]]) {
                    base.isSwipBack = YES;
                }
