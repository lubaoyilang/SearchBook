...
@property (nonatomic, strong) UIImageView *uthingLogo;
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        SVProgressHUDBackgroundColor = RGBCOLOR(137, 135, 136);
...
- (void)showProgress:(float)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    if(progress >= 0) {
        ...
        self.uthingLogo.center = self.indefiniteAnimatedView.center;
        [self.hudView addSubview:self.uthingLogo];
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    [self updatePosition];
    [self.indefiniteAnimatedView removeFromSuperview];
    [self.uthingLogo removeFromSuperview];
    ...
    [UIView animateWithDuration:0.15
                     ...
                     completion:^(BOOL finished){
                         if(self.alpha == 0.0f || self.hudView.alpha == 0.0f) {
                             ...
                             [_uthingLogo removeFromSuperview];
                             _uthingLogo = nil;
...
- (UIImageView *)uthingLogo {
    if (_uthingLogo == nil) {
        _uthingLogo = [[UIImageView alloc] initWithFrame:RECTMAKE(0, 0, 24, 24)];
        _uthingLogo.backgroundColor = [UIColor clearColor];
        _uthingLogo.image = [UIImage imageNamed:@"loading"];
        _uthingLogo.tag = 69266;
    }
    return _uthingLogo;
}
